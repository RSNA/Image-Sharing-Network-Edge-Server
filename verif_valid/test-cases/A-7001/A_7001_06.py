...
class A_7001_06():
    def run(self):
        ...
        numberOfSOPsPerSeries = 5;  # This is the number of dicom images sent per unique series
        ...
        ## --------------------------------------------------------- ##
        print 'Prepare DICOM MR & CT files to with Study, Series, SOP UIDs' 
        ## --------------------------------------------------------- ##
        studySeriesSopUIDs = [];
        ...
        ## For this test, generate two sets of files from two different templates
        mrStudySeriesSopUIDs = studySeriesSopUIDs[:5];
        success = baseTest.generateCustomDicomFiles(mrStudySeriesSopUIDs, customFieldsDictionary, testDataDir + '/dicom_files/MR.dcm', sentFilesDirectory)
        if not success:
            return (False, 'Failed to generate custom dicom MR files');
        ctStudySeriesSopUIDs = studySeriesSopUIDs[5:];
        success = baseTest.generateCustomDicomFiles(ctStudySeriesSopUIDs, customFieldsDictionary, testDataDir + '/dicom_files/CT.dcm', sentFilesDirectory)
        if not success:
            return (False, 'Failed to generate custom dicom CT files');
...
if __name__ == "__main__":
    print 'Running A_7001_06 as main()'
    test = A_7001_06();

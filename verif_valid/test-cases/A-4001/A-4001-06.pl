...
## Main starts here
 ...
 $folderDICOM = "/rsna/test-data/FUJI95714-EVRLE";
 $targetFolder= "$defaultEdgeFolder/dcm/A-4001-06";
 image_sharing::remove_folder($targetFolder);

 ($name, $patID, $accessionNumber) = ("Waters^F", "A-4001-06", "A-4001-06-ACC");
 image_sharing::cstore($folderDICOM, $ae, $dcmHost, $port, $name, $patID, $accessionNumber, "");
 print "DICOM files transmitted\n";

 @allFiles = <$targetFolder/A-4001-06-ACC/*>;
 ...
 foreach $f(@allFiles) {
  ...
  die "A-4001-06 fail: at least one file did not pass ($totalPass of $totalFiles did pass)\n" if ($totalPass != $totalFiles);
  die "A-4001-06 fail: test designed for 1 image, not $totalPass\n" if ($totalPass != 1);
  
  print "A-4001-06 pass\n";
